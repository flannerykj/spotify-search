...
function parseTrack2(track) {
  return track;
}
function parseTrack(track) {
  return {
    name: track.name,
    artist: parseArtist(track.artists[0]).name,
...
const SpotifyClient = {
  ...
  getTrack(trackId) {
    return this._get(
      SPOTIFY_BASE_URI + '/tracks/' + trackId
    ).then((data) => parseTrack(data));
  },

  getTracks(keywords) {
    return this._get(
      SPOTIFY_BASE_URI + '/search?q=' + keywords.join('+') + '&limit=5&type=track'
    ).then((data) => (
      data.tracks.items.map((t) => parseTrack(t))
    ));
  },
  getAlbums(albumIds) {
    return this._get(
